...
const CAREER_DETAIL: Record<string, {
  ...
  content: {
    ...
    trap: "처음부터 장비에 돈 쓰지 마세요.\n스마트폰으로 3개월은 충분합니다."
  },
  lecture: {
    title: "온라인 강의 / 코칭", icon: "🎓", income: "월 50만~1,000만원",
    why: "한 번 만들면 자면서도 팔리는 구조입니다. 잘하는 걸 강의로 만들면 누적 수천만원이 됩니다. 한국은 '배우는 문화'가 강해서 시장이 매우 큽니다.",
  ...
  design: {
    ...
    trap: "수정 요청이 무제한이 되지 않도록\n계약서(메시지)에 수정 횟수 명시하세요."
  },
  ai: {
    title: "AI 자동화 대행", icon: "🤖", income: "월 50만~500만원",
    why: "지금 가장 뜨는 부업입니다. ChatGPT·Claude를 활용한 업무 자동화, 챗봇 제작, 콘텐츠 자동생성 — 이걸 해줄 수 있는 사람이 턱없이 부족합니다. 요즘 가장 빠르게 성장하는 분야입니다.",
  ...
  food: {
    ...
    trap: "혼자 너무 많이 만들려다 지쳐요.\n메뉴 수와 양을 처음엔 작게 시작하세요."
  },
  realestate: {
    ...
    trap: "공부 없이 따라하다가 큰 손실을 보는 경우가 많습니다.\n공부가 제일 먼저입니다."
  },
};
...
const ROADMAP: Record<string, { m1: string; m2: string; m3: string }> = {
  ...
  consulting: { m1: "내 경험 중 남들이 돈 내고 물어볼 것 1개 정하고 크몽 '전문가 서비스'에 등록.", m2: "첫 1~3건 무료·할인가로 진행해 후기 확보, 카카오 채널로 상담 창구 열기.", m3: "후기 3개 이상 쌓이면 적정 단가로 올리고 정기 상담 고객 확보하기." },
  realestate: { m1: "경매·갭투자 관련 책 3권 읽고 관심 지역 시세 파악, 임장 시작.", m2: "소액으로 접근 가능한 매물(오피스텔 전세 끼고 등) 후보 3곳 좁히기.", m3: "세금·대출 구조 확인 후 첫 투자 검토, 무리한 레버리지는 피하기." },
};
...
const INCOME_SIM: Record<string, { h1: string; h3: string; note: string }> = {
  ...
  consulting: { h1: "월 30만~150만원", h3: "월 200만~800만원", note: "컨설팅은 시간당 단가가 가장 높은 부업이에요. 3시간이면 상담 2~3건을 소화할 수 있어요." },
  realestate: { h1: "월 10만~50만원", h3: "월 50만~무한대", note: "부동산은 준비 기간엔 수익이 거의 없다가, 첫 투자 이후 수익 곡선이 완전히 달라져요. 조급해하지 마세요." },
};
