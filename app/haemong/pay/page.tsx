  ...
  const pay = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `haemong_${Date.now()}`, phone: _ph||"", name: name.trim(), amount: AMOUNT, category: "꿈해몽 30일권", source: "haemong" }) }).catch(()=>{});
