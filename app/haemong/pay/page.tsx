  ...
  const pay = async () => {
    ...
    try {
      const res = await fetch("/api/payup/charge", {
        ...
        body: JSON.stringify({ cardNo: clean, expireMonth: expM.padStart(2,"0"), expireYear: expY.slice(-2), birthday: birth, cardPw: pw, amount: AMOUNT, itemName: "점운 꿈해몽 30일권", userName: name.trim(), mobileNumber: mobile.replace(/\D/g,"") }),
      });
      const data = await res.json();
      if (data.success) {
        try { const p = Number(localStorage.getItem("haemong_unlock_until")||0); localStorage.setItem("haemong_unlock_until", String((p>Date.now()?p:Date.now())+30*24*60*60*1000)); } catch {}
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `haemong_${Date.now()}`, phone: mobile.replace(/\D/g,"")||"", name: name.trim(), amount: AMOUNT, category: "꿈해몽 30일권", source: "haemong" }) }).catch(()=>{});
    ...
    return (
      <div style={S.wrap}>
        <div style={S.inner}>
          ...
          <p style={{ textAlign:"center", fontSize:13, color:"#9ca3af", margin:"0 0 24px" }}>꿈해몽 단독권 또는 7개앱 풀패스</p>
          <div onClick={()=>setShowForm(true)} style={{ cursor:"pointer", border:"1px solid rgba(255,255,255,0.18)", borderRadius:18, padding:"20px 18px", marginBottom:14, background:"rgba(255,255,255,0.04)" }}>
            ...
            <p style={{ fontSize:14, color:"#d1d5db", margin:0 }}>🌙 꿈해몽 30일권</p>
            <p style={{ fontSize:12, color:"#6b7280", margin:"4px 0 0" }}>결제 완료 후 꿈해몽 30일 이용 가능</p>
          </div>
          ...
          <p style={{ fontSize:11, color:"#6b7280", textAlign:"center" }}>990원 이용권은 꿈해몽 앱 하나만 30일 이용 가능합니다.</p>
        </div>
      </div>
    );
  }
    ...
    <div style={S.wrap}>
      <div style={S.inner}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
          ...
          <span style={{ fontSize:13, color:"#6b7280" }}>꿈해몽 30일 이용권</span>
        </div>
        <div style={{ background:"linear-gradient(135deg,#0f0a2e,#1a1060)", border:"1px solid rgba(124,58,237,0.4)", borderRadius:18, padding:"20px 18px", marginBottom:24, textAlign:"center" }}>
          <p style={{ fontSize:24, margin:"0 0 4px" }}>🌙</p>
          <p style={{ fontSize:16, fontWeight:900, color:"white", margin:"0 0 6px" }}>점운 꿈해몽</p>
          <p style={{ fontSize:13, color:"#9ca3af", margin:"0 0 14px", lineHeight:1.6 }}>꿈 해석 · 태몽 · 흉몽 · 길몽 전부<br />30일 동안 마음껏 이용</p>
        ...
        <p style={{ fontSize:11, color:"#6b7280", textAlign:"center", lineHeight:1.6 }}>결제 후 꿈해몽 30일 이용권이 즉시 적용돼요.<br />카드 정보는 결제 후 저장되지 않습니다.</p>
      </div>
    </div>
  );
}
