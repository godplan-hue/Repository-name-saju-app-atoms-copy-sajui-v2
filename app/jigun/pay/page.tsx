...
function PayInner() {
  ...
  const finalizeSuccess = (info: { paymentId: string; id: string; finalAmount: number; name: string; mobile: string; couponCode: string; hasCoupon: boolean }) => {
    ...
    window.location.href = info.id ? `/jigun/result/${info.id}` : "/jigun";
  };
    ...
    (async () => {
      try {
        ...
        const finalInfo = info || {
          paymentId: orderId, id, finalAmount: Number(tossAmount) || finalAmount,
          name, mobile, couponCode: coupon, hasCoupon: !!(coupon && couponData),
        };
  ...
  const payToss = async () => {
    ...
    try {
      ...
      const orderId = `jigun-toss-${Date.now()}`;
      const cleanMobile = mobile.replace(/\D/g,"");
    ...
    <div style={S.wrap}>
      <div style={S.inner}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <a href={id ? `/jigun/result/${id}` : "/jigun"} style={{ color: "#a78bfa", fontSize: 13, textDecoration: "none" }}>← 돌아가기</a>
