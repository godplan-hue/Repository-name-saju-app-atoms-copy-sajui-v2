...
function PartnerAnalysisResultInner() {
  ...
  // 고정된 키 하나만 쓰면, 새로운 고객 결과지를 보고 읽기를 눌렀을 때 이전 고객의
  // 멈춘 위치가 남아있어서 그 내용이 그대로 이어 읽히는 심각한 문제가 있었음 —
  // 고객명+생년월일+패키지를 키에 포함시켜서 다른 고객이면 절대 같은 키를 안 씀
  ...
  const toggleReadAloud = () => {
    ...
    const _isKakao = /KAKAOTALK|kakaoBrowser|KAKAO/i.test(_ua);
    if (_isKakao) {
    ...
    if (/NAVER\(inapp/i.test(_ua)) {
      try { navigator.clipboard?.writeText(window.location.href); } catch {}
      alert("이 링크를 복사해서 크롬 또는 구글 창에 붙여넣으면 읽기가 돼요.");
      return;
    }
    if (!("speechSynthesis" in window)) return;
    // window.speechSynthesis.speaking은 실제 상태와 다르게 나오는 경우가 있어서
    // 그 값으로 "정지 vs 이어읽기"를 판단하면 멈추기가 먹통이 되는 문제가 있었음 —
    // 우리가 직접 관리하는 speaking 상태만 보고 무조건 멈춤
    if (speaking) {
      window.speechSynthesis.cancel();
      setSpeaking(false);
      releaseWakeLock();
  ...
  return (
    <>
      ...
      <main style={{ minHeight: "100vh", background: BG, backgroundImage: "url('https://i.pinimg.com/736x/67/e2/18/67e218f591e7e620d3e2a4445d9527c4.jpg')", backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "fixed", fontFamily: "'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif" }}>
