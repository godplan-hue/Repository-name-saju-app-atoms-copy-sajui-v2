...
const BOSS_MIND: Record<string, string[]> = {
  fire: [
    '"나도 사실 사람이야. 다 잘 되길 바라는 거라고..."',
    '"오늘 진짜 나도 내가 왜 그랬는지 모르겠어. 스트레스가 많았나 봐."',
    '"너한테 화낸 거 아니야. 그냥 타이밍이 안 좋았어. 진짜야."',
    '"나도 알아, 내가 좀 과했다는 거. 근데 사과는 또 어색하네."',
    '"집에 가서 생각해보니 내가 오버했더라. 미안하다는 말은... 다음에 할게."',
  ],
  cold: [
    '"말 안 해도 다 알아. 근데 말하기가 어려워."',
    '"...할 말 있으면 니가 먼저 해. 난 원래 말이 없어."',
    '"표정 관리가 안 되는 게 아니라 그냥 표정이 없는 거야."',
    '"속으론 나쁘지 않게 생각하고 있어. 티가 안 날 뿐이지."',
    '"...(오늘도 별다른 말 없음. 그게 다임)"',
  ],
  micro: [
    '"완벽하게 하고 싶어서 그래. 나를 위해서가 아니라 팀을 위해서야."',
    '"다 완벽하게 하고 싶어서 그래. 너한테만 그러는 거 아니야."',
    '"이 정도 체크 안 하면 나중에 더 크게 문제 생겨. 믿어."',
    '"나도 예전에 이렇게 배웠어. 그때보다는 내가 훨씬 부드러운 편이야."',
    '"잔소리처럼 들렸다면 미안. 근데 다 너 성장하라고 하는 거야."',
  ],
  fake: [
    '"나도 살아남으려고 이러는 거야. 이해해줘."',
    '"너한테만 그러는 거 아니야. 원래 사람 좋아해서 그래."',
    '"그냥 편하게 지내자는 거지, 다른 뜻은 진짜 없어."',
    '"말이 좀 많았나? 그래도 나쁜 의도는 아니었어."',
    '"친해지고 싶어서 그런 거야. 부담 주려던 건 아니었어."',
  ],
  ghost: [
    '"내가 없는 게 최고의 복지 아닌가? 하하."',
    '"... (딱히 별생각 없음. 알아서 잘하고 있는 줄 알았음)"',
    '"무소식이 희소식이라고 생각했는데, 아니었나?"',
    '"... (사실 요즘 정신이 다른 데 가 있음)"',
    '"믿고 맡긴 거야. 근데 티는 안 냈네."',
  ],
}
...
const SMART_RESPONSE: Record<string, string[]> = {
  fire: [
    '"팀장님, 중요한 내용 같은데 메모하면서 들을게요. 잠시만요."\n감정이 격할 때는 이렇게 한 박자 늦추는 말을 건네면 상황이 누그러지는 경우가 많아요. 화내는 이유를 캐묻기보다 내용부터 정리하겠다는 태도가 훨씬 효과적이에요.',
    '"제가 놓친 부분이 있으면 바로 수정하겠습니다. 어느 부분인지 짚어주시겠어요?"\n화가 난 상황에서는 변명보다 구체적인 지점을 물어보는 게 대화를 빨리 마무리 짓는 방법이에요. 초점을 감정에서 사실로 옮기는 효과가 있어요.',
    '"네, 알겠습니다. 정리해서 다시 확인 받으러 오겠습니다."\n격앙된 상사에게는 길게 대응할수록 불이 커져요. 짧고 명확하게 받아들이는 말로 대화를 빨리 끝내는 게 오히려 나를 지키는 전략이에요.',
    '"제 잘못이면 고치겠습니다. 근데 이 부분은 사전에 논의된 적이 없어서 확인이 필요해요."\n무조건 수긍하기보다, 사실관계가 다른 부분은 침착하게 짚어주는 게 필요해요. 감정적으로 받아치지 않는 게 핵심이에요.',
    '"지금 바로 답 드리기보다 잠시 생각 정리하고 말씀드려도 될까요?"\n순간적으로 욱하는 상황에서는 즉답을 피하고 텀을 두는 것만으로도 갈등이 줄어들어요. 상사도 시간이 지나면 누그러지는 타입이에요.',
  ],
  cold: [
    '"제가 진행한 부분 어떠셨는지 짧게라도 말씀 주시면 다음에 더 잘 반영하겠습니다."\n무반응인 상사에게는 구체적인 질문으로 반응을 유도하는 게 좋아요. 예/아니오로 답할 수 있는 질문일수록 대답을 받아내기 쉬워요.',
    '"이 방향으로 계속 가도 될까요, 아니면 다르게 수정할까요?"\n선택지를 좁혀서 물어보면 짧게라도 답을 얻어낼 확률이 높아져요. 열린 질문보다 닫힌 질문이 효과적이에요.',
    '"혹시 불편하신 부분 있으면 말씀만 해주세요. 저는 괜찮으니까요."\n표현이 적은 상사에게는 먼저 마음의 문을 열어두는 말이 의외로 반응을 이끌어낼 때가 있어요. 부담 없다는 신호를 주는 게 핵심이에요.',
    '"오늘까지 정리해서 메일로 남겨두겠습니다. 확인 후 피드백 주시면 감사하겠습니다."\n말로 반응이 없으면 텍스트로 요청 흔적을 남기는 것도 방법이에요. 답이 없어도 기록이 남으니 나중에 근거가 돼요.',
    '"제 나름대로는 이렇게 이해했는데 맞을까요?"\n무응답인 상사에겐 내가 먼저 해석을 던지고 확인만 받는 방식이 편해요. 상대가 새로 말을 만들 필요가 없으니 대답률이 올라가요.',
  ],
  micro: [
    '"혹시 원하시는 방향이 있으면 미리 말씀해주시면 처음부터 그렇게 진행하겠습니다."\n사후 지적보다 사전 확인을 요청하면 잔소리 횟수 자체를 줄일 수 있어요. 상사의 기준을 먼저 파악하는 게 핵심이에요.',
    '"이 정도 디테일이면 충분할까요, 더 다듬을 부분이 있을까요?"\n지적받기 전에 먼저 확인을 요청하면 통제권을 어느 정도 가져올 수 있어요. 사전 질문이 잔소리를 예방하는 셈이에요.',
    '"체크리스트 만들어서 미리 여쭤보고 진행할게요."\n세세한 기준을 가진 상사에게는 눈에 보이는 확인 절차를 만들어주는 게 효과적이에요. 신뢰가 쌓이면 간섭이 자연스럽게 줄어들어요.',
    '"지난번 피드백 반영해서 이번엔 이렇게 준비했습니다. 어떠신가요?"\n이전 지적을 반영했다는 걸 먼저 보여주면 잔소리할 거리가 줄어들어요. 성장하고 있다는 신호를 주는 게 중요해요.',
    '"세세하게 봐주셔서 실수가 줄어드는 것 같아요. 이 부분만 다시 확인 부탁드려요."\n오히려 꼼꼼함을 긍정적으로 인정해주면서 필요한 확인만 요청하면 관계가 한결 편해져요.',
  ],
  fake: [
    '"말씀 감사해요! 근데 이 부분은 아직 확정된 게 아니라서, 나중에 다시 말씀드릴게요."\n선을 넘는 친절엔 적당히 예의 있게 거리를 두는 답변이 안전해요. 사적인 이야기는 최소한으로만 공유하는 게 좋아요.',
    '"그건 제가 따로 정리해서 필요할 때 말씀드릴게요!"\n웃으면서 화제를 업무로 돌리는 게 감정 소모 없이 거리를 두는 방법이에요. 단호함보다 자연스러운 전환이 효과적이에요.',
    '"팀장님도 바쁘실 텐데, 이 얘긴 나중에 편하실 때 다시 여쭤볼게요."\n상대를 배려하는 척하면서 자연스럽게 대화를 마무리 짓는 방법이에요. 거절이 아니라 타이밍 조정으로 느끼게 하는 게 포인트예요.',
    '"저는 그냥 일 얘기가 편해서요, 이건 다음에 천천히 얘기해요!"\n부드럽지만 명확하게 선을 긋는 말이에요. 웃으면서 말하면 관계가 서먹해지지 않으면서도 거리는 유지돼요.',
    '"신경 써주셔서 감사한데, 저는 이 정도가 딱 편해요."\n친절을 거절하는 게 아니라 내 기준을 알려주는 방식이에요. 상대의 호의를 부정하지 않으면서 경계를 정할 수 있어요.',
  ],
  ghost: [
    '"제가 이렇게 진행했는데, 방향 맞는지 한 번만 확인 부탁드립니다."\n존재감이 없는 상사에게는 먼저 다가가 확인을 요청하는 게 오히려 마음 편해요. 스스로 판단해서 진행하되, 중요한 갈림길만큼은 꼭 짚고 넘어가세요.',
    '"급한 건은 아니지만 놓치신 것 같아서 다시 한번 말씀드려요."\n무관심한 상사에게는 재차 알림을 주는 게 자연스러운 방법이에요. 감정 상하지 않게 사실만 짧게 전달하면 돼요.',
    '"결정은 제가 하되, 문제 생기면 바로 공유드릴게요."\n방임형 상사 밑에서는 스스로 판단 기준을 세우는 게 필요해요. 대신 리스크가 있는 부분만 미리 알려두면 나중에 책임 소재가 명확해져요.',
    '"이 건은 팀장님 승인이 필요한 부분이라 확인 부탁드립니다."\n중요한 건에는 반드시 승인 흔적을 남기도록 요청하는 게 중요해요. 나중에 방치로 인한 문제를 나 혼자 떠안지 않기 위한 방법이에요.',
    '"제가 알아서 진행하고 있는데, 혹시 다른 의견 있으시면 언제든 말씀해주세요."\n무관심을 서운해하기보다 자율성으로 받아들이는 태도가 정신 건강에 더 좋아요. 필요한 순간에만 확인을 구하면 충분해요.',
  ],
}
...
export default function WorkPage() {
  ...
  function answerQuiz(yes: boolean) {
    ...
    if (quizIdx + 1 >= QUIZ_LIST.length) {
      ...
      setContentVariant(Math.floor(Math.random() * 5))
      setStep('contact')
    } else {
        ...
        {step === 'result' && boss && (
          <div>
            ...
            <div style={{ background: 'white', borderRadius: 18, padding: '20px', marginBottom: 12, boxShadow: '0 4px 16px rgba(37,99,235,0.1)' }}>
              ...
              <p style={{ fontSize: 14, color: '#374151', lineHeight: 1.7, margin: 0, fontStyle: 'italic' }}>{BOSS_MIND[bossKey][contentVariant % BOSS_MIND[bossKey].length]}</p>
            </div>
            ...
            <div style={{ background: 'white', borderRadius: 18, padding: '20px', marginBottom: 12, boxShadow: '0 4px 16px rgba(37,99,235,0.1)' }}>
              ...
              <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.8, margin: 0, whiteSpace: 'pre-line' }}>{SMART_RESPONSE[bossKey][contentVariant % SMART_RESPONSE[bossKey].length]}</p>
            </div>
