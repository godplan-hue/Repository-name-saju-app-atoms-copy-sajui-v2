  ...
  const analyze = async () => {
    ...
    try {
      const res = await fetch("/api/lotto/analyze", {
        ...
        body: JSON.stringify({
          name, phone: cleanPhone, email, birthYear: Number(birthYear), birthMonth: Number(birthMonth), birthDay: Number(birthDay), marketing: marketingAgreed,
        }),
      });
