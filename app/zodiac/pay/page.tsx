...
function PayInner() {
  ...
  const pay = async (method: "CARD" | "KAKAOPAY" = "CARD") => {
    ...
    try {
      ...
      fetch("/api/v2/save-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:`zodiac_${Date.now()}`,phone:cleanMobile||"",name:name.trim()||"",amount:finalAmount,category:"별자리 심층 분석",source:"zodiac"})}).catch(()=>{});
      setUnlock();
      window.location.href = id ? `/zodiac/result/${id}` : "/zodiac";
