...
export default function ZodiacPayPage() {
  return (
...
function PayInner() {
  ...
  const finalizeSuccess = (info: { id: string; cleanMobile: string; name: string; finalAmount: number; coupon: string; hasCoupon: boolean }) => {
    ...
    fetch("/api/v2/save-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:`zodiac_${Date.now()}`,phone:info.cleanMobile||"",name:info.name.trim()||"",amount:info.finalAmount,category:"별자리 심층 분석",source:"zodiac"})}).catch(()=>{});
    setUnlock();
  ...
  const pay = async (method: "CARD" | "KAKAOPAY" = "CARD") => {
    ...
    if (isFree) {
      ...
      try {
        ...
        fetch("/api/v2/save-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:`zodiac_${Date.now()}`,phone:_ph||"",name:name.trim()||"",amount:0,category:"별자리 쿠폰",source:"zodiac"})}).catch(()=>{});
    ...
    try {
      ...
      const res = await portone.requestPayment({
        ...
        channelKey,
        paymentId: `zodiac_${Date.now()}_${Math.random().toString(36).slice(2)}`,
        orderName: "점운 별자리 심층 분석",
    ...
    <div style={S.wrap}>
      <div style={S.inner}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <a href={id ? `/zodiac/result/${id}` : "/zodiac"} style={{ color: "#93c5fd", fontSize: 13, textDecoration: "none" }}>← 돌아가기</a>
          <span style={{ fontSize: 13, color: "#6b7280" }}>별자리 심층 분석</span>
        </div>

        <div style={{ background: "linear-gradient(135deg,#0a0f2e,#1a1060)", border: "1px solid rgba(129,140,248,0.4)", borderRadius: 18, padding: "20px 18px", marginBottom: 24, textAlign: "center" }}>
          <p style={{ fontSize: 20, margin: "0 0 4px" }}>🌠</p>
          <p style={{ fontSize: 16, fontWeight: 900, color: "white", margin: "0 0 6px" }}>별자리 심층 분석</p>
          <p style={{ fontSize: 13, color: "#9ca3af", margin: "0 0 6px", lineHeight: 1.6 }}>
            💕 오늘의 연애·직업·건강 상세<br/>
            🍀 행운번호 6개 · 🔢 소울넘버 · ✨ 오라 색깔
          </p>
          <p style={{ fontSize: 12, color: "#818cf8", margin: "0 0 14px" }}>결제 후 24시간 열람 가능</p>
        ...
        <div style={{ marginBottom:16, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:16, padding:"14px 16px" }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <button onClick={applyCoupon} disabled={couponLoading} style={{ background:"rgba(102,126,234,0.3)", border:"1px solid rgba(102,126,234,0.6)", color:"#c4b5fd", fontSize:13, fontWeight:700, padding:"0 16px", borderRadius:12, cursor:"pointer", flexShrink:0 }}>
              {couponLoading?"...":"적용"}
            </button>
          </div>
        ...
        {isFree ? (
          <button onClick={()=>pay()} disabled={loading} style={{ width:"100%", background:loading?"rgba(102,126,234,0.5)":"linear-gradient(135deg,#667eea,#764ba2)", color:"white", border:"none", borderRadius:22, padding:"16px", fontSize:16, fontWeight:900, cursor:loading?"not-allowed":"pointer", marginBottom:12 }}>
            {loading?"처리 중...":"🎟 무료로 이용하기"}
          </button>
        ) : (
          <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:16 }}>
            <button onClick={()=>pay("CARD")} disabled={loading||!refundAgreed} style={{ width:"100%", background:(loading||!refundAgreed)?"rgba(102,126,234,0.5)":"linear-gradient(135deg,#667eea,#764ba2)", color:"white", border:"none", borderRadius:22, padding:"16px", fontSize:16, fontWeight:900, cursor:(loading||!refundAgreed)?"not-allowed":"pointer" }}>
