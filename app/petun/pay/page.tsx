  ...
  const pay = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `petun_${Date.now()}`, phone: _ph||"", name: name.trim(), amount: AMOUNT, category: "펫운 30일권", source: "petun" }) }).catch(()=>{});
