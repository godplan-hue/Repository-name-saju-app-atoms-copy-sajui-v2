  ...
  const pay = async () => {
    ...
    try {
      const res = await fetch("/api/payup/charge", {
        ...
        body: JSON.stringify({ cardNo: clean, expireMonth: expM.padStart(2,"0"), expireYear: expY.slice(-2), birthday: birth, cardPw: pw, amount: AMOUNT, itemName: "점운 펫운 30일권", userName: name.trim(), mobileNumber: mobile.replace(/\D/g,"") }),
      });
      const data = await res.json();
      if (data.success) {
        try { const p = Number(localStorage.getItem("petun_unlock_until")||0); localStorage.setItem("petun_unlock_until", String((p>Date.now()?p:Date.now())+30*24*60*60*1000)); } catch {}
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `petun_${Date.now()}`, phone: mobile.replace(/\D/g,"")||"", name: name.trim(), amount: AMOUNT, category: "펫운 30일권", source: "petun" }) }).catch(()=>{});
  ...
  return (
    <div style={S.wrap}>
      <div style={S.inner}>
        ...
        <div style={{ background:"linear-gradient(135deg,#1e0f00,#2e1800)", border:"1px solid rgba(234,88,12,0.4)", borderRadius:18, padding:"20px 18px", marginBottom:24, textAlign:"center" }}>
