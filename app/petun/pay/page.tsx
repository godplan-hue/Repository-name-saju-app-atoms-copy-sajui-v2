  ...
  const pay = async () => {
    ...
    try {
      const res = await fetch("/api/payup/charge", {
        ...
        body: JSON.stringify({ cardNo: clean, expireMonth: expM.padStart(2,"0"), expireYear: expY.slice(-2), birthday: birth, cardPw: pw, amount: finalAmount, itemName: "점운 펫운 30일권", userName: name.trim(), mobileNumber: mobile.replace(/\D/g,"") }),
      });
      ...
      if (data.success) {
        ...
        const _ph = mobile.replace(/\D/g,"");
        let _fbUntil = 0;
        if (_ph) { try { const _r = await fetch(`/api/phone-unlock?phone=${_ph}`); const _d = await _r.json(); if (_d.ok) _fbUntil = Number(_d.unlocks?.petun_unlock_until||0); } catch {} }
        const _local = Number(localStorage.getItem("petun_unlock_until")||0);
        ...
        if (_ph) fetch("/api/phone-unlock",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:_ph,unlocks:{petun_unlock_until:_until}})}).catch(()=>{});
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `petun_${Date.now()}`, phone: _ph||"", name: name.trim(), amount: finalAmount, category: "펫운 30일권", source: "petun" }) }).catch(()=>{});
        window.location.href = "/petun";
      } else {
    ...
    return (
      <div style={S.wrap}>
        <div style={S.inner}>
          ...
          <p style={{ textAlign:"center", fontSize:13, color:"#9ca3af", margin:"0 0 24px" }}>펫운 단독권 또는 7개앱 풀패스</p>
          <div onClick={()=>setShowForm(true)} style={{ cursor:"pointer", border:"1px solid rgba(255,255,255,0.18)", borderRadius:18, padding:"20px 18px", marginBottom:14, background:"rgba(255,255,255,0.04)" }}>
            ...
            <p style={{ fontSize:14, color:"#d1d5db", margin:0 }}>🐾 펫운 30일권</p>
            <p style={{ fontSize:12, color:"#6b7280", margin:"4px 0 0" }}>결제 완료 후 펫운 앱 30일 이용 가능</p>
          </div>
    ...
    <div style={S.wrap}>
      <div style={S.inner}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
          ...
          <span style={{ fontSize:13, color:"#6b7280" }}>펫운 30일 이용권</span>
        </div>
        <div style={{ background:"linear-gradient(135deg,#1e0f00,#2e1800)", border:"1px solid rgba(234,88,12,0.4)", borderRadius:18, padding:"20px 18px", marginBottom:16, textAlign:"center" }}>
          ...
          <p style={{ fontSize:13, color:"#9ca3af", margin:"0 0 14px", lineHeight:1.6 }}>반려동물 운세·궁합 · 음식 안전도<br />30일 동안 마음껏 이용</p>
        ...
        <button onClick={pay} disabled={loading||!refundAgreed} style={{ width:"100%", background:(loading||!refundAgreed)?"rgba(234,88,12,0.5)":"linear-gradient(135deg,#ea580c,#f97316)", color:"white", border:"none", borderRadius:22, padding:"16px", fontSize:16, fontWeight:900, cursor:(loading||!refundAgreed)?"not-allowed":"pointer", marginBottom:12 }}>
        ...
        <p style={{ fontSize:11, color:"#6b7280", textAlign:"center", lineHeight:1.6 }}>결제 후 펫운 30일 이용권이 즉시 적용돼요.<br />카드 정보는 결제 후 저장되지 않습니다.</p>
      </div>
    </div>
  );
}
