...
export default function PetunPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (phoneOk && p.name) setOwnerName(p.name);
      if (phoneOk && p.phone) setOwnerPhone(p.phone);
      if (phoneOk && p.phone) setRestorePhone(p.phone);
      if (phoneOk && p.email) setOwnerEmail(p.email);
    } catch {}
  }, []);
  ...
  return (
    <div style={S.wrap}>
      <div style={S.inner}>
        ...
        <p style={{ fontSize: 11, color: "#6b7280", textAlign: "center", marginTop: 10 }}>결제 후 24시간 이용 · 2초만에 결과 확인</p>
