...
export default function TarotPage() {
  ...
  const analyze = async () => {
    ...
    try {
      const res = await fetch("/api/tarot/analyze", {
        ...
        body: JSON.stringify({ name, phone: cleanPhone, email, birthYear, birthMonth: birthMonth || "1", birthDay: birthDay || "1", topic, marketing: marketingAgreed }),
      });
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      if (d.ok && d.unlocks?.tarot_unlock_until > Date.now()) {
        try { localStorage.setItem("tarot_unlock_until", String(d.unlocks.tarot_unlock_until)); } catch {}
        ...
        <div style={{ background: "linear-gradient(180deg,#1a0533 0%,#0f0320 100%)", paddingBottom: 40 }}>
          <div style={{ maxWidth: 440, margin: "0 auto", padding: "40px 24px 0", textAlign: "center" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              <Link href="/main-v2" style={{ color: "#c084fc", fontSize: 13, textDecoration: "none" }}>← 점운 홈</Link>
              <button onClick={() => { const d = { title: "점운 타로 — AI 타로카드 해석", text: "오늘 나에게 온 카드를 뽑아봤어요! 완전 무료 🃏", url: "https://jeomun.com/tarot" }; const _k=(window as any).Kakao; if(_k?.isInitialized()&&_k?.Share){_k.Share.sendDefault({objectType:"feed",content:{title:d.title,description:d.text,imageUrl:"https://i.pinimg.com/1200x/21/92/2c/21922cc59f29ba66e12cc4546e316079.jpg",link:{mobileWebUrl:d.url,webUrl:d.url}},buttons:[{title:"바로 보기",link:{mobileWebUrl:d.url,webUrl:d.url}},{title:"나도 해보기 →",link:{mobileWebUrl:d.url,webUrl:d.url}}]});}else{window.location.href=`kakaotalk://msg/send?text=${encodeURIComponent(d.text+'\n'+d.url)}`;}; }} style={{ fontSize: 12, color: "#c084fc", fontWeight: 700, background: "rgba(192,132,252,0.15)", border: "1px solid rgba(192,132,252,0.4)", borderRadius: 20, padding: "5px 12px", cursor: "pointer" }}>🔗 공유</button>
            </div>
