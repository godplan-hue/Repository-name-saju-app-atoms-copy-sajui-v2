  ...
  const pay = async () => {
    ...
    if (couponData && (finalAmount === 0 || couponData.fullAccess)) {
      ...
      try {
        ...
        fetch("/api/v2/save-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:`tarot_${Date.now()}`,phone:_ph||"",name:name.trim()||"",amount:0,category:"타로 쿠폰",source:"tarot"})}).catch(()=>{});
    ...
    try {
      const res = await fetch("/api/payup/charge", {
        ...
        body: JSON.stringify({ cardNo: clean, expireMonth: expM.padStart(2,"0"), expireYear: expY.slice(-2), birthday: birth, cardPw: pw, amount: finalAmount, itemName: "점운 타로 30일권", userName: name.trim(), mobileNumber: mobile.replace(/\D/g,"") }),
      });
      ...
      if (data.success) {
        ...
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `tarot_${Date.now()}`, phone: _ph||"", name: name.trim(), amount: finalAmount, category: "타로 30일권", source: "tarot" }) }).catch(()=>{});
        window.location.href = "/tarot";
      } else {
  ...
  return (
    <div style={S.wrap}>
      <div style={S.inner}>
        ...
        <div style={{ background:"linear-gradient(135deg,#1a0a2e,#2d0a50)", border:"1px solid rgba(126,34,206,0.4)", borderRadius:18, padding:"20px 18px", marginBottom:16, textAlign:"center" }}>
        ...
        <button onClick={pay} disabled={loading||!refundAgreed} style={{ width:"100%", background:(loading||!refundAgreed)?"rgba(126,34,206,0.5)":"linear-gradient(135deg,#7e22ce,#a855f7)", color:"white", border:"none", borderRadius:22, padding:"16px", fontSize:16, fontWeight:900, cursor:(loading||!refundAgreed)?"not-allowed":"pointer", marginBottom:12 }}>
