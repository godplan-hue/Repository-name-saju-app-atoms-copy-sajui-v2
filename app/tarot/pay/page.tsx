  ...
  const pay = async () => {
    ...
    try {
      const res = await fetch("/api/payup/charge", {
        ...
        body: JSON.stringify({ cardNo: clean, expireMonth: expM.padStart(2,"0"), expireYear: expY.slice(-2), birthday: birth, cardPw: pw, amount: AMOUNT, itemName: "점운 타로 30일권", userName: name.trim(), mobileNumber: mobile.replace(/\D/g,"") }),
      });
      const data = await res.json();
      if (data.success) {
        try { const p = Number(localStorage.getItem("tarot_unlock_until")||0); localStorage.setItem("tarot_unlock_until", String((p>Date.now()?p:Date.now())+30*24*60*60*1000)); } catch {}
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `tarot_${Date.now()}`, phone: mobile.replace(/\D/g,"")||"", name: name.trim(), amount: AMOUNT, category: "타로 30일권", source: "tarot" }) }).catch(()=>{});
      ...
      <div style={S.wrap}>
        <div style={S.inner}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:28 }}>
            <a href="/tarot" style={{ color:"#a78bfa", fontSize:13, textDecoration:"none" }}>← 돌아가기</a>
          ...
          <p style={{ textAlign:"center", fontSize:13, color:"#9ca3af", margin:"0 0 24px" }}>타로 단독권 또는 7개앱 풀패스</p>
          <div onClick={()=>setShowForm(true)} style={{ cursor:"pointer", border:"1px solid rgba(255,255,255,0.18)", borderRadius:18, padding:"20px 18px", marginBottom:14, background:"rgba(255,255,255,0.04)" }}>
            ...
            <p style={{ fontSize:14, color:"#d1d5db", margin:0 }}>🃏 타로 30일권</p>
            <p style={{ fontSize:12, color:"#6b7280", margin:"4px 0 0" }}>결제 완료 후 타로 앱 30일 이용 가능</p>
          </div>
          ...
          <p style={{ fontSize:11, color:"#6b7280", textAlign:"center" }}>990원 이용권은 타로 앱 하나만 30일 이용 가능합니다.</p>
        </div>
      </div>
    );
  }
    ...
    <div style={S.wrap}>
      <div style={S.inner}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
          ...
          <span style={{ fontSize:13, color:"#6b7280" }}>타로 30일 이용권</span>
        </div>
        <div style={{ background:"linear-gradient(135deg,#1a0a2e,#2d0a50)", border:"1px solid rgba(126,34,206,0.4)", borderRadius:18, padding:"20px 18px", marginBottom:24, textAlign:"center" }}>
          <p style={{ fontSize:24, margin:"0 0 4px" }}>🃏</p>
          <p style={{ fontSize:16, fontWeight:900, color:"white", margin:"0 0 6px" }}>점운 타로</p>
          <p style={{ fontSize:13, color:"#9ca3af", margin:"0 0 14px", lineHeight:1.6 }}>AI 타로카드 · 연애·직업·재물 상담<br />30일 동안 마음껏 이용</p>
        ...
        <button onClick={pay} disabled={loading} style={{ width:"100%", background:loading?"rgba(126,34,206,0.5)":"linear-gradient(135deg,#7e22ce,#a855f7)", color:"white", border:"none", borderRadius:22, padding:"16px", fontSize:16, fontWeight:900, cursor:loading?"not-allowed":"pointer", marginBottom:12 }}>
        ...
        <div style={{ marginBottom:16, padding:"12px 14px", background:"rgba(251,191,36,0.08)", borderRadius:12, border:"1px solid rgba(251,191,36,0.3)" }}>
          ...
          <p style={{ margin:0, fontSize:11, color:"rgba(255,255,255,0.7)", lineHeight:1.7 }}>
            · 이용권은 <strong style={{ color:"#fbbf24" }}>결제한 브라우저(기기)에서만</strong> 적용돼요.<br />
            · PC에서 결제하면 PC에서, 폰에서 결제하면 폰에서만 이용 가능해요.<br />
            · 다른 기기·브라우저에서 이용하려면 이용권을 별도로 구매해주세요.<br />
            · 디지털 콘텐츠 특성상 <strong style={{ color:"#fbbf24" }}>환불이 불가</strong>합니다.
          </p>
        </div>
        <p style={{ fontSize:11, color:"#6b7280", textAlign:"center", lineHeight:1.6 }}>결제 후 타로 30일 이용권이 즉시 적용돼요.<br />카드 정보는 결제 후 저장되지 않습니다.</p>
      </div>
    </div>
  );
}
