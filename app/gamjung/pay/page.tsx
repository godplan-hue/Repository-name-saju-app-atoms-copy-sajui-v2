  ...
  const pay = async () => {
    ...
    try {
      const res = await fetch("/api/payup/charge", {
        ...
        body: JSON.stringify({ cardNo: clean, expireMonth: expM.padStart(2,"0"), expireYear: expY.slice(-2), birthday: birth, cardPw: pw, amount: finalAmount, itemName: "점운 감정일기 30일권", userName: name.trim(), mobileNumber: mobile.replace(/\D/g,"") }),
      });
      ...
      if (data.success) {
        ...
        const _ph = mobile.replace(/\D/g,"");
        let _fbUntil = 0;
        if (_ph) { try { const _r = await fetch(`/api/phone-unlock?phone=${_ph}`); const _d = await _r.json(); if (_d.ok) _fbUntil = Number(_d.unlocks?.gamjung_unlock_until||0); } catch {} }
        ...
        if (_ph) fetch("/api/phone-unlock",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:_ph,unlocks:{gamjung_unlock_until:_until}})}).catch(()=>{});
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `gamjung_${Date.now()}`, phone: _ph||"", name: name.trim(), amount: finalAmount, category: "감정일기 30일권", source: "gamjung" }) }).catch(()=>{});
        window.location.href = "/gamjung";
      } else {
  ...
  return (
    <div style={S.wrap}>
      <div style={S.inner}>
        ...
        <button onClick={pay} disabled={loading||!refundAgreed} style={{ width:"100%", background:(loading||!refundAgreed)?"rgba(124,58,237,0.5)":"linear-gradient(135deg,#7c3aed,#ec4899)", color:"white", border:"none", borderRadius:22, padding:"16px", fontSize:16, fontWeight:900, cursor:(loading||!refundAgreed)?"not-allowed":"pointer", marginBottom:12 }}>
