  ...
  const pay = async (method: "CARD" | "KAKAOPAY" = "CARD") => {
    ...
    try {
      ...
      const res = await portone.requestPayment({
        ...
        channelKey,
        paymentId: `gamjung_${Date.now()}_${Math.random().toString(36).slice(2)}`,
      ...
      let _fbUntil = 0;
      if (cleanMobile) { try { const _r = await fetch(`/api/phone-unlock?phone=${cleanMobile}`); const _d = await _r.json(); if (_d.ok) _fbUntil = Number(_d.unlocks?.gamjung_unlock_until||0); } catch {} }
      ...
      if (cleanMobile) fetch("/api/phone-unlock",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:cleanMobile,unlocks:{gamjung_unlock_until:_until}})}).catch(()=>{});
      fetch("/api/v2/save-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:`gamjung_${Date.now()}`,phone:cleanMobile||"",name:name.trim()||"",amount:finalAmount,category:"감정일기 30일권",source:"gamjung"})}).catch(()=>{});
      window.location.href = "/gamjung";
