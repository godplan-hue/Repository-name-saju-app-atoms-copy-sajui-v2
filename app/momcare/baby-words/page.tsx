    ...
    <div style={{ minHeight: "100vh", background: "#f0f7ff", fontFamily: "'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif" }}>
      <nav style={{ background: "white", borderBottom: "1px solid #e5e7eb", padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        {unlocked ? (
          ...
          <a href="/momcare/pay" style={{ background: "#fed7aa", color: "#c2410c", borderRadius: 12, padding: "8px 14px", fontSize: 12, fontWeight: 700, textDecoration: "none" }}>{expired ? "🔒 재활성화" : "🔒 결제하기"}</a>
        )}
      </nav>
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "24px 20px" }}>
        ...
        {!hasPhone && (
          <div style={{ background: "#fef3c7", border: "1px solid #f59e0b", borderRadius: 12, padding: "14px 16px", marginBottom: 16, fontSize: 12, color: "#92400e", lineHeight: 1.6 }}>
            ⚠️ 전화번호를 등록하면 모든 기기에서 말 사전을 영구 보관해요.<br />
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              ...
              <button onClick={() => {
                ...
                try {
                  const profile = JSON.parse(localStorage.getItem("v2_saved_profile") || "{}");
                  profile.phone = ph;
                ...
                setMcUserId(uid);
                setHasPhone(true);
                fetch(`/api/momcare/save?userId=device_${localStorage.getItem("momcare_device_id")}&type=words`)
                  .then(r => r.json()).then(d => { if (Array.isArray(d.data) && d.data.length > 0) { setWords(d.data); localStorage.setItem("momcare_words", JSON.stringify(d.data)); } }).catch(() => {});
