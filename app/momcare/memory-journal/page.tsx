...
export default function MemoryJournalPage() {
  ...
  if (!unlocked) return (
    <div style={{ minHeight: "100vh", background: "#f0f7ff", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px", fontFamily: "'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif" }}>
      <div style={{ background: "white", borderRadius: 24, padding: "32px 24px", textAlign: "center", maxWidth: 340, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
        ...
        <Link href="/momcare/pay" style={{ display: "block", background: "linear-gradient(135deg, #f59e0b, #ef4444)", color: "white", borderRadius: 14, padding: "14px", fontSize: 15, fontWeight: 900, textDecoration: "none", marginBottom: 12, textAlign: "center" }}>💳 육아일기 결제하기 (1,980원·5,900원) →</Link>
  ...
  function save(m: Milestone[]) {
    ...
    if (mcUserId) {
      postWithRetry("/api/momcare/save", { userId: mcUserId, type: "milestones", data: m }).then(ok => setSyncFailed(!ok));
    }
  }
