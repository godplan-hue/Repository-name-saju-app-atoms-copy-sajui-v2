...
export default function MomcarePayPage() {
    ...
    (async () => {
      try {
        ...
        const finalInfo = info || {
          ...
          name, cleanMobile: mobile.replace(/\D/g,""), coupon, hasCoupon: !!couponData,
        };
        await finalizeSuccess(finalInfo);
      } catch {
  ...
  const finalizeSuccess = async (info: { paymentId: string; finalAmount: number; name: string; cleanMobile: string; coupon: string; hasCoupon: boolean }) => {
    ...
    let _fbUntil = 0;
    if (info.cleanMobile) { try { const _r = await fetch(`/api/phone-unlock?phone=${info.cleanMobile}`); const _d = await _r.json(); if (_d.ok) _fbUntil = Number(_d.unlocks?.momcare_unlock_until||0); } catch {} }
    const _local = Number(localStorage.getItem("momcare_unlock_until")||0);
    const _p = Math.max(_local, _fbUntil);
    const _until = (_p>Date.now()?_p:Date.now())+30*24*60*60*1000;
    try { localStorage.setItem("momcare_unlock_until", String(_until)); } catch {}
    if (info.cleanMobile) fetch("/api/phone-unlock",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:info.cleanMobile,unlocks:{momcare_unlock_until:_until}})}).catch(()=>{});
    fetch("/api/v2/save-payment",{method:"POST",headers:{"Content-Type":"application/json"},keepalive:true,body:JSON.stringify({id:info.paymentId,phone:info.cleanMobile||"",name:info.name.trim()||"",amount:info.finalAmount,category:"육아일기 30일권",source:"momcare"})}).catch(()=>{});
    window.location.href = "/momcare";
  };
  ...
  const pay = async (method: "CARD" | "KAKAOPAY" = "CARD") => {
    ...
    if (isFree) {
      ...
      try {
        ...
        fetch("/api/v2/save-payment",{method:"POST",headers:{"Content-Type":"application/json"},keepalive:true,body:JSON.stringify({id:`momcare_${Date.now()}`,phone:_ph||"",name:name.trim()||"",amount:0,category:"육아일기 쿠폰",source:"momcare"})}).catch(()=>{});
    ...
    try {
      ...
      const paymentId = `momcare_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  ...
  const payToss = async () => {
    ...
    try {
      ...
      const orderId = `momcare-toss-${Date.now()}`;
      const cleanMobile = mobile.replace(/\D/g,"");
      ...
      <div style={S.wrap}>
        <div style={S.inner}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:28 }}>
            <a href="/momcare" style={{ color:"#a78bfa", fontSize:13, textDecoration:"none" }}>← 돌아가기</a>
          ...
          <p style={{ textAlign:"center", fontSize:13, color:"#9ca3af", margin:"0 0 24px" }}>육아일기 단독권 또는 4개앱 풀패스</p>
          ...
          <div onClick={()=>setShowForm(true)} style={{ cursor:"pointer", border:"1px solid rgba(255,255,255,0.18)", borderRadius:18, padding:"20px 18px", marginBottom:24, background:"rgba(255,255,255,0.04)" }}>
            ...
            <p style={{ fontSize:14, color:"#d1d5db", margin:0 }}>👶 육아일기 30일권</p>
            <p style={{ fontSize:12, color:"#6b7280", margin:"4px 0 0" }}>추가 결제 시 기간 연장</p>
          </div>
          <p style={{ fontSize:11, color:"#6b7280", textAlign:"center" }}>1,980원 이용권은 육아일기 앱 하나만 30일 이용 가능합니다.</p>
        </div>
      </div>
    );
  }
    ...
    <div style={S.wrap}>
      <div style={S.inner}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
          ...
          <span style={{ fontSize:13, color:"#6b7280" }}>육아일기 30일 이용권</span>
        </div>
        <div style={{ background:"linear-gradient(135deg,#2e0a20,#4a0a30)", border:"1px solid rgba(190,24,93,0.4)", borderRadius:18, padding:"20px 18px", marginBottom:16, textAlign:"center" }}>
          <p style={{ fontSize:24, margin:"0 0 4px" }}>👶</p>
          <p style={{ fontSize:16, fontWeight:900, color:"white", margin:"0 0 6px" }}>점운 육아일기</p>
          <p style={{ fontSize:13, color:"#9ca3af", margin:"0 0 14px", lineHeight:1.6 }}>AI 육아 · 아기 일기 · 타임캡슐<br />30일 동안 마음껏 이용</p>
        ...
        {isFree ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:16 }}>
            <button onClick={()=>pay("CARD")} disabled={loading||!refundAgreed} style={{ width:"100%", background:(loading||!refundAgreed)?"rgba(190,24,93,0.5)":"linear-gradient(135deg,#be185d,#ec4899)", color:"white", border:"none", borderRadius:22, padding:"16px", fontSize:16, fontWeight:900, cursor:(loading||!refundAgreed)?"not-allowed":"pointer" }}>
        ...
        <p style={{ fontSize:11, color:"#6b7280", textAlign:"center", lineHeight:1.6 }}>결제 후 육아일기 30일 이용권이 즉시 적용돼요.</p>
      </div>
    </div>
  );
}
