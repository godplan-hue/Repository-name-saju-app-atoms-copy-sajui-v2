  ...
  const pay = async () => {
    ...
    try {
      const res = await fetch("/api/payup/charge", {
        ...
        body: JSON.stringify({ cardNo: clean, expireMonth: expM.padStart(2,"0"), expireYear: expY.slice(-2), birthday: birth, cardPw: pw, amount: AMOUNT, itemName: "점운 맘케어 30일권", userName: name.trim(), mobileNumber: mobile.replace(/\D/g,"") }),
      });
      ...
      if (data.success) {
        ...
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `momcare_${Date.now()}`, phone: _ph||"", name: name.trim(), amount: AMOUNT, category: "맘케어 30일권", source: "momcare" }) }).catch(()=>{});
  ...
  return (
    <div style={S.wrap}>
      <div style={S.inner}>
        ...
        <div style={{ background:"linear-gradient(135deg,#2e0a20,#4a0a30)", border:"1px solid rgba(190,24,93,0.4)", borderRadius:18, padding:"20px 18px", marginBottom:24, textAlign:"center" }}>
