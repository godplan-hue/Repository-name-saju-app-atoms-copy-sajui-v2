...
export default function MomcarePayPage() {
  ...
  const pay = async () => {
    ...
    try {
      ...
      if (data.success) {
        try { const p = Number(localStorage.getItem("momcare_unlock_until")||0); localStorage.setItem("momcare_unlock_until", String((p>Date.now()?p:Date.now())+30*24*60*60*1000)); } catch {}
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `momcare_${Date.now()}`, phone: mobile.replace(/\D/g,"")||"", name: name.trim(), amount: AMOUNT, category: "맘케어 30일권", source: "momcare" }) }).catch(()=>{});
