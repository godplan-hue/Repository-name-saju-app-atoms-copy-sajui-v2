...
export default function MomcarePayPage() {
  ...
  const finalizeSuccess = async (info: { paymentId: string; finalAmount: number; name: string; cleanMobile: string; coupon: string; hasCoupon: boolean }) => {
    ...
    fetch("/api/v2/save-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:info.paymentId,phone:info.cleanMobile||"",name:info.name.trim()||"",amount:info.finalAmount,category:"육아일기 30일권",source:"momcare"})}).catch(()=>{});
  ...
  const pay = async (method: "CARD" | "KAKAOPAY" = "CARD") => {
    ...
    if (isFree) {
      ...
      try {
        ...
        fetch("/api/v2/save-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:`momcare_${Date.now()}`,phone:_ph||"",name:name.trim()||"",amount:0,category:"육아일기 쿠폰",source:"momcare"})}).catch(()=>{});
