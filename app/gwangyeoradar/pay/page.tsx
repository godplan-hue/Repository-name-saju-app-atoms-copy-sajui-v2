...
export default function GwangyeoradarPayPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", background: "#071019" }} />}>
      <PayInner />
    </Suspense>
  );
}
...
function PayInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const finalInfo = info || { paymentId: orderId, id, finalAmount: Number(tossAmount) || finalAmount, name, mobile, couponCode: coupon, hasCoupon: !!(coupon && couponData) };
  ...
  const payToss = async () => {
    ...
    try {
      ...
      const orderId = `gwangyeoradar-toss-${Date.now()}`;
      const cleanMobile = mobile.replace(/\D/g, "");
      const pendingInfo = { paymentId: orderId, id, finalAmount, name, mobile, couponCode: coupon, hasCoupon: !!(coupon && couponData) };
  ...
  const pay = async (method: "CARD" | "KAKAOPAY" = "CARD") => {
    ...
    if (isFree) {
      ...
      try {
        ...
        window.location.href = id ? `/gwangyeoradar/result/${id}?paid=1` : "/gwangyeoradar";
      } finally { setLoading(false); }
      return;
    }
    setLoading(true); setError("");
    try {
      ...
      const paymentId = `gwangyeoradar_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    ...
    <div style={S.wrap}>
      <div style={S.inner}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <a href={id ? `/gwangyeoradar/result/${id}` : "/gwangyeoradar"} style={{ color: "#67e8f9", fontSize: 13, textDecoration: "none" }}>← 돌아가기</a>
          <span style={{ fontSize: 13, color: "#6b7280" }}>연락기록통계 정밀분석</span>
        </div>

        <div style={{ background: "linear-gradient(135deg,#04222f,#0c3a4d)", border: "1px solid rgba(34,211,238,0.4)", borderRadius: 18, padding: "20px 18px", marginBottom: 24, textAlign: "center" }}>
          <p style={{ fontSize: 24, margin: "0 0 6px" }}>📡</p>
          <p style={{ fontSize: 16, fontWeight: 900, color: "white", margin: "0 0 6px" }}>연락기록통계 정밀분석 전체 공개</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, margin: "14px 0 16px", textAlign: "left" }}>
            {["📶 연락 패턴 27개 지표", "🌡 관계 온도·변화율", "🧭 놓친 신호 발견", "⚠️ 위험 신호 진단", "🔁 회복 가능성", "🧾 최종 행동 처방전"].map(item => (
              <div key={item} style={{ background: "rgba(255,255,255,0.07)", borderRadius: 10, padding: "8px 10px", fontSize: 12, color: "#a5f3fc" }}>{item}</div>
            ))}
          </div>
          <p style={{ fontSize: 12, color: "#67e8f9", margin: "0 0 14px" }}>결제 후 바로 열림 · 24시간 이용</p>
        ...
        <div style={{ marginBottom: 16, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "14px 16px" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={applyCoupon} disabled={couponLoading} style={{ background: "rgba(8,145,178,0.3)", border: "1px solid rgba(8,145,178,0.6)", color: "#a5f3fc", fontSize: 13, fontWeight: 700, padding: "0 16px", borderRadius: 12, cursor: "pointer", flexShrink: 0 }}>
              {couponLoading ? "..." : "적용"}
            </button>
          </div>
        ...
        {isFree ? (
          <button onClick={() => pay()} disabled={loading} style={{ width: "100%", background: loading ? "rgba(8,145,178,0.5)" : "linear-gradient(135deg,#0891b2,#0ea5e9)", color: "white", border: "none", borderRadius: 22, padding: "16px", fontSize: 16, fontWeight: 900, cursor: loading ? "not-allowed" : "pointer", marginBottom: 12 }}>
            {loading ? "처리 중..." : "결제하기"}
          </button>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
            <button onClick={() => pay("CARD")} disabled={loading || !refundAgreed} style={{ width: "100%", background: (loading || !refundAgreed) ? "rgba(8,145,178,0.5)" : "linear-gradient(135deg,#0891b2,#0ea5e9)", color: "white", border: "none", borderRadius: 22, padding: "16px", fontSize: 16, fontWeight: 900, cursor: (loading || !refundAgreed) ? "not-allowed" : "pointer" }}>
