...
function V2ResultInner() {
  ...
  const [showPcGuideModal, setShowPcGuideModal] = useState(false);
  const toggleMusic = () => {
  ...
  const toggleReadAloud = () => {
    ...
    if (readChunksRef.current.length === 0) {
      ...
      // 파트너 서브도메인에서는 990원 결제 유도 섹션 자체를 숨기므로 읽기에서도 제외
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "20px 16px 80px" }}>
          ...
          {(tier === "free" || tier === "select") && profile?.birthYear && (() => {
            ...
            return (
              <div style={{ margin: "0 12px 12px", borderRadius: 16, overflow: "hidden", border: "1.5px solid rgba(236,72,153,0.12)" }}>
                <div style={{ background: G, color: "white", padding: "10px 16px", fontSize: 13, fontWeight: 900 }}>🔮 {profile?.name}님의 사주팔자 맛보기</div>
              ...
              <div style={{ margin: "0 12px 12px", background: "#fdf6e3", borderRadius: 16, overflow: "hidden", border: "1.5px solid rgba(217,180,80,0.45)" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, padding: "14px 16px" }}>
                  ...
                  <div style={{ background: BG, borderRadius: 12, padding: "10px 6px", textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 12, fontWeight: 900, color: "#1a1a2e" }}>{gan}({gan === "갑" ? "甲" : gan === "을" ? "乙" : gan === "병" ? "丙" : gan === "정" ? "丁" : gan === "무" ? "戊" : gan === "기" ? "己" : gan === "경" ? "庚" : gan === "신" ? "辛" : gan === "임" ? "壬" : "癸"})</div>
                  </div>
                </div>
              ...
              <div style={{ background: "white", borderRadius: 24, border: "1.5px solid rgba(255,215,0,0.4)", marginBottom: 12, overflow: "hidden" }}>
                ...
                <div style={{ padding: "16px 18px 20px" }}>
                  ...
                  <div style={{ background: "rgba(255,215,0,0.12)", borderRadius: 10, padding: "10px 12px", filter: "blur(3px)", userSelect: "none", pointerEvents: "none" }}>
                    <p style={{ fontSize: 10, color: "#d4af37", fontWeight: 800, margin: "0 0 6px" }}>🔮 3,900원 또는 9,900원 이상 구매 시 공개</p>
                  ...
                  <p style={{ fontSize: 12, color: "#dc2626", fontWeight: 800, margin: "12px 0 0", textAlign: "center", fontStyle: "italic", wordBreak: "keep-all" }}>👉 {profile.name}님의 정확한 변화 시점과 구체적인 실행법이 <span style={{ display: "inline-block", background: "#ec4899", color: "white", fontWeight: 900, fontStyle: "normal", padding: "2px 10px", borderRadius: 8, margin: "0 2px" }}>3,900원 또는 9,900원 이상</span> 결제 시 모두 공개됩니다</p>
          ...
          <div style={{ marginBottom: 16, borderRadius: 18, background: "linear-gradient(135deg, #fff7ed, #fef3c7)", border: "1.5px solid rgba(245,158,11,0.35)", padding: "20px 18px" }}>
            {couponCode ? (
              ...
              <>
                ...
                <p style={{ fontSize: 12, color: "#78350f", margin: "0 0 12px" }}>연락처만 남기면 바로 쿠폰코드를 드려요!<br />(1인 1회, 중복 불가)</p>
        ...
        {tier === "free" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
            ...
            <button onClick={() => { nav(`/main-v2/pay?amount=4900&next=${encodeURIComponent("/payment-complete?special=sinyeon_premium&paid=4900")}`); }}
          ...
          <div style={{ display: "flex", gap: 10, margin: "10px 0" }}>
            <div onClick={() => { window.open("/haemong", "_blank"); }} style={{ flex: 1, borderRadius: 14, cursor: "pointer", background: "#fff", border: "2px solid #dc2626", overflow: "hidden", boxShadow: "0 3px 12px rgba(220,38,38,0.15)" }}>
              ...
              <div style={{ padding: "10px" }}>
                ...
                <p style={{ fontSize: 10, color: "#6b7280", margin: "0 0 8px", lineHeight: 1.5 }}>사주 결제하면<br />꿈해몽+점냥이+Q&amp;A 3종 24시간 무료</p>
