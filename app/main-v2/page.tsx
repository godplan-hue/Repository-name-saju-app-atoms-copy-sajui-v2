...
const BANNERS = [
  ...
  {
    appBanner: true,
    directUrl: "/main-v2/taegil",
    bg: "linear-gradient(135deg,#451a03 0%,#92400e 50%,#78350f 100%)",
    bgImg: "https://i.pinimg.com/736x/d2/bb/fc/d2bbfc237cf31aa0ac32e4f1fe9934d3.jpg",
    badge: "📅 택일 — 길일 찾기",
    badgeBg: "#b45309",
    icon: "📅",
    tagline: "이사·결혼·개업 최적 날짜",
    desc: "사주로 찾는 나만의 길일\n이사·결혼·개업·입학 택일",
    cta: "택일 보러가기 →",
    ctaColor: "#fde68a",
    route: "free" as const,
  },
  {
    ...
    badgeBg: "#1d4ed8",
    icon: "⭐",
    tagline: "내 인생의 큰 흐름을 확인",
    desc: "지금 내 대운이 언제 바뀌나?\n10년 단위 인생 타이밍 분석",
    cta: "대운 보러가기 →",
    ctaColor: "#93c5fd",
    route: "free" as const,
  },
  {
    ...
    desc: "사주·꿈운·직운·합운·궁합·MBTI·행운번호·펫운·육아일기·감정일기·타로·별자리·다이어트·가계부·이상형월드컵·인생이영화라면·추구미·직장버티기·연락기록통계·손절각",
    cta: "전체앱 보기 →",
    ctaColor: "#c4b5fd",
    route: "free" as const,
  },
...
const FORTUNE_CATEGORIES = [
  { id: "free",          title: "오늘의 무료운세", sub: "오늘의 운세 매일 무료",          emoji: "🌟", img: "https://i.pinimg.com/1200x/2f/1b/4e/2f1b4e0713ac39d9090ae3a3e5862db9.jpg", bg: "linear-gradient(145deg, #dcfce7, #bbf7d0)", accent: "#16a34a", price: "무료",        priceBg: "#15803d" },
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
          {FORTUNE_CATEGORIES.map(cat => (
            <div
              ...
              onTouchEnd={e => { const t = e.currentTarget as HTMLElement; const dx = Math.abs(e.changedTouches[0].clientX - parseFloat(t.dataset.sx||"0")); const dy = Math.abs(e.changedTouches[0].clientY - parseFloat(t.dataset.sy||"0")); if (dy > 10 && dy >= dx) { e.preventDefault(); return; } if (dx < 10) { e.preventDefault(); onPick(cat.id); } }}
              style={{
                aspectRatio: "1 / 1",
...
const EXTRA_ITEMS = [
  { id: "sinyeon",    label: "신년운세",   sub: "올해 내 사주 완전분석", emoji: "🎍", price: "₩990",   priceNum: 990,  premium: false, img: "https://i.pinimg.com/1200x/a6/30/a7/a630a728391bc4784a814211184b42bb.jpg", accent: "#16a34a" },
  ...
  { id: "love_detail",      label: "연애사주",         sub: "나의 연애 DNA",           emoji: "💗", price: "₩990",   priceNum: 990,  premium: false, img: "https://i.pinimg.com/1200x/f1/66/b5/f166b50a65fc824659d395a75037937b.jpg", accent: "#be185d" },
  ...
  { id: "taegil",           label: "택일(擇日)",       sub: "내 사주에 맞는 좋은날", emoji: "📅", price: "₩2,900", priceNum: 2900, premium: false, img: "https://i.pinimg.com/1200x/b7/c3/ca/b7c3ca787665a1ec29ea1d2643ddc55a.jpg", accent: "#0284c7" },
...
const EXTRA_MCFG: Record<string, { emoji: string; title: string; desc: string; price: string; priceNum: number; features: string[] }> = {
  ...
  divorce:         { emoji: "🌧", title: "이혼운세", desc: "현재 관계 에너지·분리 흐름·새 출발 타이밍", price: "₩990", priceNum: 990, features: ["🌊 지금 관계 에너지", "📋 분리 과정 흐름", "💰 재산·현실 측면", "🌅 이후 새 출발", "💪 앞으로를 위한 조언"] },
    ...
    <div style={{ padding: "0 14px 20px", maxWidth: 480, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
        {EXTRA_ITEMS.map(item => (
          <div key={item.id} onClick={() => onPick(item.id)} onTouchStart={e => { const t = e.currentTarget as HTMLElement; t.dataset.sx = String(e.touches[0].clientX); t.dataset.sy = String(e.touches[0].clientY); }} onTouchEnd={e => { const t = e.currentTarget as HTMLElement; const dx = Math.abs(e.changedTouches[0].clientX - parseFloat(t.dataset.sx||"0")); const dy = Math.abs(e.changedTouches[0].clientY - parseFloat(t.dataset.sy||"0")); if (dy > 10 && dy >= dx) { e.preventDefault(); return; } if (dx < 10) { e.preventDefault(); onPick(item.id); } }} style={{ aspectRatio: "1 / 1", borderRadius: 14, cursor: "pointer", position: "relative", overflow: "hidden", boxShadow: `0 3px 12px ${item.accent}28`, WebkitTransform: "translateZ(0)", transform: "translateZ(0)" }}>
...
export default function MainV2() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#fdf6ec", backgroundImage: `url('https://i.pinimg.com/736x/a8/9c/11/a89c118880aebf98b375dacf833715f1.jpg')`, backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "fixed", fontFamily: "'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif", overflowX: "hidden" }}>
      ...
      {/* 오픈기념 5종 세트 990원 고정특가 + 심층분석 990원 특가(신규 별도박스) — 메인 첫 화면에서 바로 보이고 바로 결제되게 */}
      {!isPartner && (
        <div style={{ padding: "0 16px", margin: "4px 0 16px", maxWidth: 600, marginLeft: "auto", marginRight: "auto", display: "flex", gap: 8 }}>
          <button
            onClick={() => {
      ...
      <footer style={{ padding: "0 30px 24px", textAlign: "center" }}>
        ...
        <div style={{ color: "#581c87", fontSize: 10.5, fontWeight: 700, lineHeight: 1.8, marginBottom: 12, letterSpacing: "0.1px" }}>
          ...
          <p style={{ margin: 0 }}>대표전화 010-2106-2689 ·<br />유선 031-585-7255</p>
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <a href="mailto:info@jeomun.com?subject=점운 문의" style={{ display: "inline-block", padding: "6px 16px", border: "1.5px solid #ec4899", borderRadius: 20, color: "#ec4899", textDecoration: "none", fontWeight: 800, fontSize: 11 }}>📧 이메일 문의</a>
          </div>
        </div>
      ...
      {(() => {
        const MCFG: Record<string, { emoji: string; title: string; desc: string; price: string; features?: string[]; catKey?: string; catKeys?: string[]; priceNum?: number; preselect?: string }> = {
          ...
          naming:        { emoji: "💫", title: "심층 5개 묶음 특가", desc: "재물·연애·건강·성공·총운 5개 묶음 · 총 ₩3,900", price: "₩3,900", features: ["💰 재물운", "💕 연애운", "💪 건강운", "🎯 성공운", "✨ 총운"], priceNum: 3900 },
          wealth5:       { emoji: "💰", title: "운세 골라담기", desc: "원하는 운세만 골라서 · 개당 ₩3,900", price: "₩3,900/개", features: ["💰 재물운", "💕 연애운", "💪 건강운", "🎯 성공운", "✨ 총운"] },
          deep990:       { emoji: "💰", title: "오픈기념 심층분석 특가", desc: "원하는 운세만 골라서 · 개당 ₩990 (정가 ₩3,900)", price: "₩990/개", features: ["💰 재물운", "💕 연애운", "💪 건강운", "🎯 성공운", "✨ 총운"] },
            ...
            <div onClick={e => e.stopPropagation()} style={{ background: "#16103a", border: "1px solid rgba(167,139,250,0.2)", borderRadius: "16px 16px 0 0", padding: "20px 20px 32px", width: "100%", maxWidth: 420 }}>
              ...
              <button
                onClick={() => {
                  ...
                  if (showModal === "deep990") {
                    if (modalSelectedCats.length === 0) return;
                    const d990Paid = modalSelectedCats.length * 990;
                    const d990Label = modalSelectedCats.length === 1 ? (modalSelectedCats[0].split(" ").slice(1).join(" ") || "운세") : `${modalSelectedCats.length}개 운세 묶음`;
                    localStorage.setItem("v2_paid_cats", JSON.stringify(modalSelectedCats));
                    const _d990Next = `/payment-complete?package=${encodeURIComponent(d990Label)}&pages=${modalSelectedCats.length * 30}&paid=${d990Paid}`;
                    window.location.href = `/main-v2/pay?amount=${d990Paid}&next=${encodeURIComponent(_d990Next)}`;
                    return;
                  }
                  if (showModal === "naming") {
