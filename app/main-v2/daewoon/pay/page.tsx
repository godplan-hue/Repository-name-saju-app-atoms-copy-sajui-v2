  ...
  const payToss = async () => {
    ...
    try {
      ...
      await payment.requestPayment({
        ...
        failUrl: `${window.location.origin}${window.location.pathname}${window.location.search}${window.location.search ? "&" : "?"}tossFail=1`,
        customerName: name.trim() || "고객",
        customerMobilePhone: cleanMobile || undefined,
      });
      // successUrl/failUrl로 페이지가 이동하므로 여기 도달하지 않음
    } catch (err: any) {
    ...
    <main style={{
      ...
      <div style={{ maxWidth: 400, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
          ...
          <button onClick={payToss} disabled={loading || !refundAgreed}
            style={{ width: "100%", padding: "16px 0", background: (loading || !refundAgreed) ? "rgba(0,100,255,0.3)" : "#0064FF", color: "#fff", border: "none", borderRadius: 50, fontWeight: 900, fontSize: 16, cursor: (loading || !refundAgreed) ? "not-allowed" : "pointer", boxShadow: (loading || !refundAgreed) ? "none" : "0 6px 22px rgba(0,100,255,0.35)" }}>
            {loading ? "결제 중..." : `토스로 결제 ₩${price.toLocaleString()}`}
          </button>
        </div>
