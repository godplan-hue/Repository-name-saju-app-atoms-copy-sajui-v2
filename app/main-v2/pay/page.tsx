...
function PayInner() {
  ...
  const finalizeSuccess = (info: {
    ...
    if (info.displayAmount > 0) {
      let packageLabel = "운세";
      try {
        const qIdx = info.next.indexOf("?");
        if (qIdx !== -1) {
          const pkgParam = new URLSearchParams(info.next.slice(qIdx + 1)).get("package");
          if (pkgParam) packageLabel = pkgParam;
        }
      } catch {}
    ...
    const withPid = `${info.next}${info.next.includes("?") ? "&" : "?"}pid=${encodeURIComponent(info.paymentId)}`;
    const targetUrl = info.isTaegil ? `${withPid}&taegilPaid=1` : withPid;
    try {
    ...
    (async () => {
      try {
        ...
        const finalInfo = info || {
          paymentId: orderId, amount, displayAmount: Number(tossAmount) || displayAmount,
          name, mobile, email, couponCode, discountPct, next, isTaegil, sourceInfo: "토스페이먼츠",
        };
  ...
  useEffect(() => {
    ...
    // 결제 시작 정보가 5분 이내로 신선하면 저장해둔 정보로 완료 처리를 이어감.
    // pay_pending은 전체 앱이 공유하는 localStorage 키라서 info.page로 이 페이지에서
    // 시작한 결제가 맞는지 반드시 확인 — 아니면 다른 앱 결제가 잘못 완료 처리될 수 있음
    ...
    if (info.page === "mainv2" && fromPaymentGateway && isFresh) {
      sessionStorage.removeItem("pay_pending");
      try { localStorage.removeItem("pay_pending"); } catch {}
      finalizeSuccess(info);
    }
  ...
  // 토스페이먼츠 통합결제(카드+간편결제+가상계좌) — jeomun72su 직계약 MID
  const payToss = async () => {
    if (!refundAgreed) { setError("아래 체크박스를 먼저 체크해주세요. ✅"); return; }
    if (!mobile.replace(/\D/g, "") || mobile.replace(/\D/g, "").length < 10) { setError("전화번호를 입력해주세요."); return; }
    setLoading(true); setError("");
    try {
      ...
      await payment.requestPayment({
        method: "CARD",
        amount: { currency: "KRW", value: displayAmount },
        orderId,
        orderName: "점운 사주 분석",
    ...
    <main style={{ minHeight: "100vh", background: "linear-gradient(180deg,#1a0835,#0d0520)", color: "white", fontFamily: "'Apple SD Gothic Neo','Malgun Gothic',sans-serif", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px 16px" }}>
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <button
          onClick={payToss}
          disabled={loading}
          style={{ width: "100%", padding: "15px 0", background: loading ? "rgba(0,100,255,0.3)" : "#0064FF", color: "#fff", border: "none", borderRadius: 50, fontWeight: 900, fontSize: 16, cursor: loading ? "not-allowed" : "pointer", boxShadow: loading ? "none" : "0 6px 22px rgba(0,100,255,0.35)", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
        >
          {loading ? "결제 중..." : `토스로 결제 ₩${displayAmount.toLocaleString()}`}
        </button>

        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 10, textAlign: "center", margin: "8px 0 0" }}>SSL 보안 결제 · NHN KCP · 카카오페이 · 토스페이먼츠</p>
        </>)}
      </div>
    </main>
  );
