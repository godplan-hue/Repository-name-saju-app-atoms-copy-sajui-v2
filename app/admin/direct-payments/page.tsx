                  ...
                  {leads.filter(l => sourceFilter === "all" || (l.sources ?? [l.source ?? "free"]).includes(sourceFilter)).map((lead, i) => (
                    <tr key={lead.id} style={{ borderBottom: "1px solid #f3f4f6", background: i % 2 === 0 ? "white" : "#fafafa" }}>
                      <td style={{ padding: "10px 12px", fontWeight: 700 }}>
                        {lead.name}
                        <span style={{color:"#f00",fontSize:9,display:"block"}}>{JSON.stringify({n:lead.name,t:typeof lead.name,k:Object.keys(lead).filter(k=>["name","phone","source","sources"].includes(k))})}</span>
                      </td>
