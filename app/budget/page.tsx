...
function getTodayCount() {
  ...
  return (base + (block >= 1 ? 390 + (lcg % 180) : 0) + (block >= 2 ? 450 + ((lcg >> 4) % 230) : 0)).toLocaleString();
}
...
export default function BudgetPage() {
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      const hasTicket = d.ok && d.unlocks?.budget_unlock_until > Date.now();
      if (hasTicket) {
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#0f172a", color: "#f5f5f5", fontFamily: "'Apple SD Gothic Neo','Malgun Gothic',sans-serif" }}>
        ...
        <div style={{ maxWidth: 420, margin: "0 auto", padding: "48px 20px" }}>
          ...
          <p style={{ color: "#9ca3af", fontSize: 13, marginTop: 8, textAlign: "center" }}>오늘 <strong style={{ color: "#fbbf24" }}>{count}</strong>명이 가계부를 기록했어요</p>
        </div>
      ...
      <div style={{ maxWidth: 440, margin: "0 auto", padding: "20px 16px 110px" }}>
        ...
        {budgetExpiringSoon && (
          <div style={{ background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 12, padding: "12px 14px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap" as const, gap: 8 }}>
            ...
            <a href="/budget/pay" style={{ background: "#f97316", color: "white", fontSize: 12, fontWeight: 900, padding: "6px 14px", borderRadius: 14, textDecoration: "none" }}>1,980원으로 30일 연장 →</a>
          </div>
        )}
        ...
        {budgetLocked && !budgetNeverPaid && (
          <div style={{ background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.3)", borderRadius: 12, padding: "12px 14px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap" as const, gap: 8 }}>
            ...
            <a href="/budget/pay" style={{ background: "linear-gradient(135deg, #f97316, #fb923c)", color: "white", fontSize: 12, fontWeight: 900, padding: "7px 14px", borderRadius: 14, textDecoration: "none" }}>1,980원으로 재활성화 →</a>
          </div>
        )}
