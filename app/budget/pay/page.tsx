  ...
  const pay = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const _p = Number(localStorage.getItem("budget_unlock_until")||0);
        ...
        fetch("/api/v2/save-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: `budget_${Date.now()}`, phone: _ph||"", name: name.trim(), amount: AMOUNT, category: "가계부 30일권", source: "budget" }) }).catch(()=>{});
