...
export default function BudgetPayPage() {
  ...
  const finalizeSuccess = async (info: { paymentId: string; finalAmount: number; name: string; cleanMobile: string; coupon: string; hasCoupon: boolean }) => {
    ...
    let _fbUntil = 0;
    if (info.cleanMobile) { try { const _r = await fetch(`/api/phone-unlock?phone=${info.cleanMobile}`); const _d = await _r.json(); if (_d.ok) _fbUntil = Number(_d.unlocks?.budget_unlock_until||0); } catch {} }
  ...
  const pay = async (method: "CARD" | "KAKAOPAY" = "CARD") => {
    ...
    try {
      ...
      const paymentId = `budget_${Date.now()}_${Math.random().toString(36).slice(2)}`;
