...
function PayInner() {
  ...
  const pay = async (method: "CARD" | "KAKAOPAY" = "CARD") => {
    ...
    try {
      ...
      fetch("/api/v2/save-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:`mbti_${Date.now()}`,phone:cleanMobile||"",name:name.trim()||"",amount:finalAmount,category:"MBTI 심층 분석",source:"mbti"})}).catch(()=>{});
      const _until = Date.now() + 24 * 60 * 60 * 1000;
      try { localStorage.setItem("mbti_unlock_until", String(_until)); } catch {}
      if (cleanMobile) fetch("/api/phone-unlock",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:cleanMobile,unlocks:{mbti_unlock_until:_until}})}).catch(()=>{});
      window.location.href = id ? `/mbti/result/${id}?paid=1` : "/mbti";
