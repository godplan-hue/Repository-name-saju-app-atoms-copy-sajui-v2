        ...
        <>
          ...
          <div ref={buyModalRef} style={{ position: "fixed", bottom: 0, left: 0, right: 0, margin: "0 auto", maxWidth: 480, background: "white", borderRadius: "20px 20px 0 0", maxHeight: "88vh", overflowY: "auto", overscrollBehavior: "contain", padding: "12px 14px 20px", zIndex: 301, willChange: "transform", transform: "translateZ(0)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 5, marginBottom: 10 }}>
              {SPECIAL_990.map(s => (
                <button key={s.id}
                  onClick={() => { nav(`/main-v2/pay?amount=${s.paid}&next=${encodeURIComponent(`/payment-complete?special=${s.id}&paid=${s.paid}`)}`); }}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 5, marginBottom: 10 }}>
              {SPECIAL_2900.map(s => (
                <button key={s.id}
                  onClick={() => {
                    if ((s as any).daeun) { nav(`/main-v2/daewoon`); return; }
                    if (s.id === "taegil") { nav(`/main-v2/taegil`); return; }
                    if (s.id === "reunion" || s.id === "pet_compat") { setAwaitOther({ id: s.id, label: s.label }); return; }
                    nav(`/main-v2/pay?amount=2900&next=${encodeURIComponent(`/payment-complete?special=${s.id}&paid=2900`)}`);
                  }}
              ...
              {OLD_SINGLES.map(s => (
                <button key={s.id}
                  onClick={() => {
                    ...
                    nav(`/main-v2/pay?amount=3900&next=${encodeURIComponent(`/payment-complete?package=${encodeURIComponent(s.label)}&pages=30&paid=3900`)}`);
                  }}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6, marginBottom: 10 }}>
              {PKGS.map(p => {
                ...
                return (
                  <button key={p.id}
                    onClick={() => { nav(`/main-v2/pay?amount=${p.paid}&next=${encodeURIComponent(`/payment-complete?package=${encodeURIComponent(p.label)}&pages=${p.pages}&paid=${p.paid}`)}`); }}
          ...
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, margin: "0 auto", maxWidth: 480, background: "white", borderRadius: "20px 20px 0 0", padding: "20px 18px 32px", zIndex: 303 }}>
            ...
            <button
              onClick={() => {
                ...
                setAwaitOther(null);
                setOtherInput("");
                nav(`/main-v2/pay?amount=2900&next=${encodeURIComponent(`/payment-complete?special=${awaitOther.id}&paid=2900`)}`);
              }}
